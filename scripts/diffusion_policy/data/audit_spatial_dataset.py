...
def planned_straight_goal(speed: float, height: float) -> np.ndarray:
    return np.asarray(
        [
            0.5 * speed,
            0.0,
...
def goal_support(
    ...
    for speed in speeds:
        for height in heights:
            goal = planned_straight_goal(speed, height)
...
def main() -> None:
    ...
    support = goal_support(goals, normalizer, list(args.speeds), list(args.heights))
